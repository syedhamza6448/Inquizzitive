...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const hoverTargets = document.querySelectorAll(
    "a, button, .illustration, .detail-quiz, .quiz-link, button[type='submit'], .floating-icons",
  )
  ...
  hoverTargets.forEach((el) => {
    el.addEventListener("mouseenter", () => {
      ...
      // Custom logic for different elements
      let labelText
      if (el.classList.contains("detail-quiz")) {
        labelText = "More Details"
      } else if (el.classList.contains("floating-icons")) {
        labelText = el.getAttribute("data-label") || "Interactive"
      } else {
